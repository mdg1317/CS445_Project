...
//displays a message to user's screen saying they joined
...
socket.on('chat-message', data => {
  //displays the message to the user's screen along with the name of the sender
  displayMessage(`${data.name}: ${data.message}: ${data.date}`)
})
...
messageForm.addEventListener('submit', e => {
  ...
  //when this happens, we call displayMessage to post some text to their screen saying that they sent a message
...
function displayMessage(message) {
  //set variable equal to the string that displayMessage is being called on
